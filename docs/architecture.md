...
| [`_auth/storage.py`](../src/notebooklm/_auth/storage.py) | **One deep module for the whole `storage_state.json` seam** (ADR-0033 sanctioned merge — absorbed `storage_writer.py`, `storage_transaction.py` and `_auth/_browser_cookie_filter.py`), in labelled sections: (1) the file-lock primitives; (2) secure-parent-dir prep + the platform-neutral bounded acquire; (3) the write transaction template `in_storage_transaction` + its `raise_`/`skip_`/`report_on_lock_unavailable` policies (ADR-0031 Stage 3); (4) the snapshot types; (5) the cookie snapshot/delta CAS merge math plus `save_cookies_to_storage`, the monkeypatchable delegate seam; (6) the value-free writer outcome types; (7) the **write-time cookie-domain filter** `filter_storage_state_cookies_by_domain_policy` + its value-free malformed-row diagnostics, applied under the lock by three of the writers as ADR-0029's entry-path-independent guarantee (the capture arms' own call is NOT redundant with it — theirs runs before `heal_captured_state` so the PSIDTS heal sees domain-filtered rows; ADR-0033 D3 keeps both); (8) the **canonical `storage_state.json` writer** (ADR-0029) — the only `_auth` module that performs the atomic write, with an intent-shaped API (CAS merge, account metadata, login/re-mint full replace, master-token persist) on the unified bounded storage lock; (9) the **account records** (`SECTION 7b`, relocated from `_auth/account.py` by ADR-0033 PR 5.2) — `read_account_metadata` and the `_sanitize_legacy_account_record` derivation that carries ADR-0029's anti-wrong-account contract, the detached one-shot legacy promotion with its `atexit` drain, `write_account_metadata` / `clear_account_metadata`, and `_drop_legacy_account_key`, the sibling `context.json` privacy scrub (the one writer here that does NOT touch `storage_state.json`). The write boundary is pinned at *function* granularity by `tests/_guardrails/test_storage_writer_boundary.py`. |
...
| [`_auth/_browser_cookie_filter.py`](../src/notebooklm/_auth/_browser_cookie_filter.py) | **Shim** — re-exports `filter_storage_state_cookies_by_domain_policy` from `_auth/storage.py`; defines nothing. Removed at the next major. (ADR-0033 PR 4.2 relocated the filter to `storage.py`: it is write-time policy — three of its six call sites are intent writers — and only carried a `browser_` name because the capture arms were its first callers.) |
...
| [`_auth/account.py`](../src/notebooklm/_auth/account.py) | **Account NETWORK identity**: probing `?authuser=N` for every Google account a cookie jar can authenticate as (`enumerate_accounts` / `_probe_authuser`), pulling the active user's email out of a NotebookLM page (`extract_email_from_html`), and formatting the `authuser` wire value (`format_authuser_value` / `authuser_query`). Plus `repair_account_metadata_from_playwright_storage`, which recomposes over both halves (probe, then persist). The account **record** itself — read/write/promote/scrub of the persisted `{authuser, email}` binding — is persistence and lives in `_auth/storage.py` (ADR-0033 PR 5.2), which retired the bidirectional lazy-import pair the split needed; the surviving edge is one-way, `account` -> `storage`, at module scope. |
...
| [`_auth/refresh.py`](../src/notebooklm/_auth/refresh.py) | Token refresh driver (external login command, secret redaction). Coalesces refresh-cmd runs across loops via the `single_flight` core and serializes the subprocess across processes with a per-path refresh flock ([refresh-2]). Owns the cold-start fallback **sequence** in one place (`_cold_fallbacks`: the L2.5 refresh-cmd rung, then L3/L4 cold recovery — ADR-0030's documented order, which cold start was aligned to in a dedicated behavior-change PR; a failed L2.5 falls through to the re-mint rungs instead of ending the ladder) while `recovery.py` keeps the flight-leader body. Also owns `_resolve_token_route_kwargs`, the token-route glue absorbed from the deleted `_auth/headers` module (ADR-0033 sanctioned merge — it held one function whose only call sites are the entry points here). Its three replaceable collaborators — the refresh-cmd subprocess runner, the cross-process flock acquirer, and the refresh lock-path deriver — are injected through the frozen `RefreshDeps` record (threaded keyword-only from the entry points to the flight-leader body, each field resolved LATE so an unset field still tracks the module namespace) rather than through module-scope aliases tests monkeypatch. |
...
| `_auth/_browser_cookie_filter.py` | Shim: re-exports the write-time cookie-domain filter from `storage.py` (removed at next major) |
...
│   ├── _browser_cookie_filter.py # Shim: re-exports the write-time cookie-domain filter from storage.py (removed at next major)
